    ...
    <div className="p-4 border-l w-1/3">
      ...
      <div className="flex space-x-2 mt-2">
        {tabs.map(t => (
          <button
            ...
            className={`px-3 py-1 rounded ${activeTab === t.key ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"}`}
          >
      ...
      <ul className="mt-3 space-y-1">
        {tabs.find(t => t.key === activeTab)?.data.map(item => {
          const diff = item.countedQty !== undefined ? item.countedQty - item.systemQty : null;
          return (
