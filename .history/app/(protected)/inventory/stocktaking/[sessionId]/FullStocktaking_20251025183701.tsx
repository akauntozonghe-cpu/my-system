...
export default function FullStocktaking({ sessionId }: { sessionId: string }) {
  ...
  if (isNumeric) {
    ...
    // QRコード読み取り → location / majorCategory / minorCategory で検索
    const matches = items.filter(
      (i) =>
        i.location === code ||
        i.majorCategory === code ||
        i.minorCategory === code
    );
              ...
              {sortedItems.map((item: Item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="border px-2 py-1">{item.majorCategory ?? "-"} / {item.minorCategory ?? "-"}</td>
