  ...
  const handleScan = async (code: string) => {
    ...
    if (isJan) {
      ...
      if (matches.length > 0) {
        ...
        setErrorMessage(`バーコード「${code}」は登録されていません。読み取ったバーコードを確認してください。`);
        setScanMode("error");
        await logScanError(code, "バーコード");
      }
    } else if (code.startsWith("http://") || code.startsWith("https://")) {
      setErrorMessage(`「${code}」は登録されていません。これはURL形式です。読み取った内容を確認してください。`);
      setScanMode("error");
      await logScanError(code, "URL");
    } else {
      ...
      if (matches.length > 0) {
        ...
        setErrorMessage(`QRコード「${code}」は登録されていません。読み取ったQRコードを確認してください。`);
        setScanMode("error");
        await logScanError(code, "QRコード");
      }
    }
  };
