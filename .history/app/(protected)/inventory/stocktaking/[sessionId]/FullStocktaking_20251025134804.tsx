...
export default function FullStocktaking({ sessionId }: { sessionId: string }) {
  ...
  // スキャン関連
  const [scanMode, setScanMode] = useState<"idle" | "barcode" | "qr">("idle");
  ...
  const handleScan = (code: string, type: "barcode" | "qr") => {
    if (type === "barcode") {
      ...
      // QR → meta.target を更新（例: 保管場所や分類）
      setMeta((prev) =>
        prev ? { ...prev, target: { mode: "qr", values: [code] } } : prev
      );
      setScanMode("qr");
    }
  };
    ...
    <div>
      ...
      {scanMode === "idle" && (
        <div className="space-x-4 mt-4">
          <button
            onClick={() => handleScan("4901234567890", "barcode")}
            className="px-4 py-2 bg-blue-600 text-white rounded"
          >
            バーコード読み取り
          </button>
          <button
            onClick={() => handleScan("QR-LOCATION-001", "qr")}
