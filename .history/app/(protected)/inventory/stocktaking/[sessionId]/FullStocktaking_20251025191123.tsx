...
export default function FullStocktaking({ sessionId }: { sessionId: string }) {
  ...
  const handleScan = async (code: string) => {
    ...
    if (isNumeric) {
      // バーコード読み取り → janCode で検索
      const matches = items.filter((i) => i.janCode === code);
      ...
      // QRコード → location / categoryLarge / categorySmall で検索
      const matches = items.filter(
  (i) =>
    String(i.location) === String(code) ||
    String(i.categoryLarge) === String(code) ||
    String(i.categorySmall) === String(code)
);
  ...
  if (!confirmed) {
    return (
      <div className="p-6 border rounded bg-white shadow">
        ...
        <p className="mb-4">
          大分類:{" "}
          {meta?.target?.mode === "categoryLarge"
            ? meta.target.values.join(", ")
            : "全体"}
        </p>
        <button
      ...
      {(scanMode === "barcode" || scanMode === "qr") && currentItem && (
        <div className="text-center mt-6">
          ...
          <p className="text-sm text-gray-600">
            ...
            {currentItem?.location ?? "-"} ／ 大分類: {currentItem?.categoryLarge ?? "-"} ／ 小分類: {currentItem?.categorySmall ?? "-"}
          </p>
          <input
            type="number"
            min={0}
